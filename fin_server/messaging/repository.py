...
class MessagingRepository:
    ...
    def __init__(self):
        ...
        logger.debug("MessagingRepository initialized (lazy loading)")
    ...
    def _ensure_collections(self):
        ...
        # Log only warnings for unavailable collections
        if self._conversations_repo is None:
            logger.warning("conversations repo unavailable")
        if self._messages_repo is None:
            logger.warning("chat_messages repo unavailable")
    ...
    def create_conversation(self, conversation: Conversation) -> str:
        ...
        if self.conversations is None:
            raise RuntimeError("Conversations collection unavailable")

        doc = conversation.to_db_doc()
        doc['created_at'] = datetime.utcnow()
        doc['last_activity'] = datetime.utcnow()

        logger.debug(f"create_conv: id={conversation.conversation_id}, participants={len(conversation.participants)}")
        ...
        if conversation.conversation_type == ConversationType.DIRECT:
            ...
            if existing:
                ...
                logger.debug(f"existing direct conv: {existing_id}")
                return existing_id

        result = self.conversations.insert_one(doc)
        conv_id = conversation.conversation_id
        logger.info(f"conv created: {conv_id}")
    ...
    def _add_conversation_to_users(self, conversation_id: str, participants: list):
        ...
        if self.user_conversations is None:
            logger.debug("user_conversations unavailable, skip")
            return
        ...
        for user_key in participants:
            try:
                ...
                logger.error(f"add_conv_to_user failed: {user_key}, {e}")
    ...
    def get_conversation(self, conversation_id: str, user_key: str = None) -> Optional[Dict]:
        ...
        if self.conversations is None:
            logger.warning("conversations unavailable")
            return None

        logger.debug(f"get_conv: {conversation_id[:12]}...")
        ...
        if conv and user_key:
            ...
            if not is_participant:
                logger.debug(f"user {user_key[:8]}... not participant")
                return None
    ...
    def send_message(self, message: Message) -> str:
        ...
        logger.debug(f"send_msg: conv={message.conversation_id[:12]}...")
    ...
    def delete_message(self, message_id: str, user_key: str, for_everyone: bool = False) -> bool:
        """Delete a message. Sets TTL for auto-purge after 7 days."""
        if self.messages is None:
            return False

        expires_at = datetime.utcnow() + timedelta(days=MESSAGE_TTL_DAYS)
        ...
        if for_everyone:
            ...
            logger.debug(f"msg {message_id[:8]}... deleted, ttl={MESSAGE_TTL_DAYS}d")
        else:
            msg = self.messages.find_one({'message_id': message_id})
            if msg:
                ...
                if participants and deleted_for >= participants:
                    ...
                    logger.debug(f"msg {message_id[:8]}... deleted by all, ttl={MESSAGE_TTL_DAYS}d")
                else:
                    result = self.messages.update_one(
                        {'message_id': message_id},
    ...
    def clear_conversation(self, conversation_id: str, user_key: str, for_everyone: bool = False) -> int:
        """Clear all messages in a conversation. Sets TTL for auto-purge."""
        if self.messages is None:
            return 0
        ...
        if for_everyone:
            ...
            logger.info(f"conv {conversation_id[:12]}... cleared: {count} msgs, ttl={MESSAGE_TTL_DAYS}d")
...
def get_messaging_repository() -> MessagingRepository:
    ...
    if not _messaging_repo.is_available():
        logger.debug("messaging repo collections unavailable")
