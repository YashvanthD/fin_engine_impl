...
import {
  Typography, TextField, Button, Stack, CircularProgress, MenuItem, Select, InputLabel, FormControl, Chip
} from '@mui/material';
import { getAccessToken, getUserInfo } from '../../utils/auth';
import { apiFetch } from '../../utils/api';

export default function AddUser({ onSubmit, isDialog, initialData }) {
  ...
  const [form, setForm] = useState(normalizeUserData(initialData || {
    username: '',
    email: '',
    phone: '',
    password: '',
  ...
  React.useEffect(() => {
    if (initialData) setForm(normalizeUserData(initialData));
  }, [initialData]);
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Stack spacing={2}>
        <Typography variant="h6" fontWeight={700} gutterBottom>Add User</Typography>
